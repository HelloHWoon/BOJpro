...
/*
이 문제는 시간 제한이 있어 시간복잡도를 줄여야 한다.
이전에 모든 좌석을 순회하면서 해당 좌석의 W*W 범위만큼 조사했던 방식은
시간복잡도가 O(R*C*W^2) 만큼 나와 매우 비효율적이었다.
따라서 방식을 모든 좌석이 아닌 친구가 앉아있는 좌석 주변에 가중치를 더하는 방식으로
풀어내었다.
이렇게 하면 시간 복잡도가 O(N*W^2)로 기존 방식에 비해 약 10배 줄어든 시간복잡도를 보인다.
*/
